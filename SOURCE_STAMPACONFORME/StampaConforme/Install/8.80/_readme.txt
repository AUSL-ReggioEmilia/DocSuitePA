Per un corretto aggiornamento pulire il contenuto della directory bin del servizio prima dell'installazione.
...
Ogni file di configurazione deve essere modificato con le impostazione attuali del servizio recuperate dal file di configurazione.
 ...
 - stampaconforme.appsettings.config (Lista dei parametri di appSettings).
 - stampaconforme.system.servicemodel.bindings.config (Lista dei bindings di configurazione).
 - stampaconforme.system.servicemodel.client.config (Impostare l'indirizzo di base del servizio converter windows).
 - EnterpriseLibrary.Logging.config.

#AC
################################################
E' obbligatorio sovrascrivere il file Web.config con quello fornito nel setup
di installazione.

#AC
################################################
E' possibile rimuovere il parametro UserOverprintConverter dal file stampaconforme.appsettings.config in quanto
ora risulta abilitato di default.
...
Per la configurazione dei servizi Windows converter di Office e OpenOffice si rimanda ai README specifici presenti nella
directory Install.
