...
namespace VecompSoftware.JeepService.PosteWeb.Service
{
    public class WsLettera
    {
        ...
        public WsLettera(string userName, string password, X509Certificate certificate, string url, double timeoutMilliseconds, PosteWebParameters parameters)
        {
            ...
            _lolService.ClientCertificates.Add(certificate);
            _lolService.Timeout = (int)timeoutMilliseconds;
        ...
        public string GetIdRichiesta(LOLRequest lettera)
        {
            RecuperaIdRichiestaResult res = _lolService.RecuperaIdRichiesta();
            ...
            {
                throw new PosteWebException("Impossibile creare richiesta lettera: ({0}) {1} - {2}.", res.CEResult.Type, res.CEResult.Code, res.CEResult.Description);
            }
        ...
        {
            ValidaDestinatariResults res = _lolService.ValidaDestinatari(idRichiesta, recipients);
        ...
        public InvioResult Send(string idRichiesta, string customer, LOLSubmit submission)
        {
            InvioResult res = _lolService.Invio(idRichiesta, customer, submission);
        ...
        {
            ValorizzaSingleResult result = _lolService.ValorizzaSingle(enquiry);
        ...
        public PreConfermaResult Confirm(Richiesta[] requests)
        {
            ...
            PreConfermaResult result = _lolService.PreConferma(requests, true);
        ...
        public static Richiesta[] InstantiateRequests(IList<LOLRequest> lettere)
        {
            var requests = new List<Richiesta>(lettere.Count);
            requests.AddRange(lettere.Select(InstantiateRequest));
            return requests.ToArray();
        }

        public static Richiesta InstantiateRequest(LOLRequest lettera)
        {
            return new Richiesta { GuidUtente = lettera.GuidPoste, IDRichiesta = lettera.IdRichiesta };
        }

        public LOLSubmit CreaSubmit(LOLRequest message)
        {
