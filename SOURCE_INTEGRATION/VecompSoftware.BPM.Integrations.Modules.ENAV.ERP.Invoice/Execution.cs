...
namespace VecompSoftware.BPM.Integrations.Modules.***REMOVED***.ERP.Invoice
{
    ...
    public class Execution : ModuleBase
    {
        ...
        private readonly decimal _databaseTenantId = 0;
            ...
            try
            {
                ...
                foreach (ReceivableInvoice receivableInvoice in _dbContext.ReceivableInvoices.Where(f => f.TenantId == _databaseTenantId && f.WorkflowProcessed == null && f.RecordOwner == ReceivableInvoice.ERP_OWNER))
                {
                    ...
                    try
                    {
                        ...
                        //TODO: verificare se è un bug perché qui sta facendo una insert UDS al posto di UPDATE
        ...
        private async Task WorkflowStartPayableInvoiceAsync(PayableInvoice payableInvoice)
        {
            ...
            try
            {
                ...
                WorkflowReferenceModel workflowReferenceModelProtocol = await CreateProtocolBuildModelAsync(payableInvoice, invoiceProtocolFileModel, container, contact, udsRepository, roleModels, protocolUniqueId, udsID, invoiceConfiguration.ProtocolCategoryId,
                    correlationId, invoice_metadatas, invoiceAttachments, invoiceConfiguration.WorkflowRepositoryName);
                WorkflowReferenceModel workflowReferenceModelPECMail = CreatePECMailBuildModel(payableInvoice, invoicePECMailModel, container, contact, pecMailBox, invoiceConfiguration.MailRecipients, udsRepository, roleModels, protocolUniqueId,
        ...
        private async Task UpdateInvoiceMetadataAsync(ReceivableInvoice receivableInvoice)
        {
            ...
            {
                Encoding encoding = EncodingUtil.GetEncoding(receivableInvoice.AutoInvoice);
                ArchiveDocument archiveDocument = await _documentClient.InsertDocumentAsync(new ArchiveDocument()
                {
                    Archive = _workflowLocation.ProtocolArchive,
                    ContentStream = encoding.GetBytes(receivableInvoice.AutoInvoice),
        ...
        private async Task BuildReceivableInvoiceAsync(DocumentUnit protocolDocumentUnit, IDictionary<string, object> uds_metadatas, UDSRepository uDSRepository, Contact contact, List<RoleModel> roleModels)
        {
            ...
            {
                Archive = _workflowLocation.ProtocolArchive,
                ContentStream = documentContent.Blob,
                Name = biblosDocument.Name,
            });
            ...
            udsBuildModel.WorkflowActions.Add(new WorkflowActionDocumentUnitLinkModel(
                new DocumentUnitModel() { UniqueId = protocolUniqueId, Environment = (int)DSWEnvironmentType.Protocol },
                new DocumentUnitModel() { UniqueId = udsID, Environment = uDSRepository.DSWEnvironment, IdUDSRepository = uDSRepository.UniqueId }));
            CommandInsertUDSData commandInsertUDSData = new CommandInsertUDSData(_moduleConfiguration.TenantName, _moduleConfiguration.TenantId, _moduleConfiguration.TenantAOOId, _identityContext, udsBuildModel);
        ...
        private async Task WorkflowProtocolBuildCompleteCallback(IEventCompleteProtocolBuild evt, IDictionary<string, object> properties)
        {
            try
            {
                ...
                using (ERPDbContext dbContext = new ERPDbContext(_logger, ModuleConfigurationHelper.JsonSerializerSettings, _moduleConfiguration.ConnectionString))
                {
                    ...
                    _logger.WriteInfo(new LogMessage($"Setting Protocol reference {protocolBuildModel.Protocol.Year}/{protocolBuildModel.Protocol.Number:0000000} to invoice WorkflowInstanceId {evt.CorrelationId}"), LogCategories);
