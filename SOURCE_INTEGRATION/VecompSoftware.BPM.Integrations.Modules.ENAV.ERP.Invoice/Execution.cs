...
using VecompSoftware.BPM.Integrations.Modules.ENAV.ERP.Invoice.Configurations;
using VecompSoftware.BPM.Integrations.Modules.ENAV.ERP.Invoice.Models;
...
namespace VecompSoftware.BPM.Integrations.Modules.ENAV.ERP.Invoice
{
    ...
    public class Execution : ModuleBase
    {
            ...
            : base(logger, ModuleConfigurationHelper.MODULE_NAME)
        {
            ...
            {
                _logger.WriteError(new LogMessage("ENAV.ERP.Invoice -> Critical error in costruction module"), ex, LogCategories);
                throw;
            }
        }
        #endregion
        ...
        protected override void Execute()
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.WriteError(new LogMessage("VecompSoftware.BPM.Integrations.Modules.ENAV.ERP.Invoice -> Critical Error"), ex, LogCategories);
                throw;
            }
        }
        ...
        protected override void OnStop()
        {
            CleanSubscriptions();
            _dbContext.Dispose();
            _logger.WriteInfo(new LogMessage("OnStop -> ENAV.ERP.Invoice"), LogCategories);
        }
        ...
        private async Task WorkflowStartPayableInvoiceAsync(PayableInvoice payableInvoice)
        {
            ...
            try
            {
                ...
                if (!workflowResult.IsValid || !workflowResult.InstanceId.HasValue)
                {
                    ...
                    throw new Exception("VecompSoftware.BPM.Integrations.Modules.ENAV.ERP.Invoice");
                }
        ...
        private async Task<T> RetryingPolicyActionAsync<T>(Func<Task<T>> func, int step = 1, int retry_tentative = 10)
        {
            ...
            {
                _logger.WriteError(new LogMessage("VecompSoftware.BPM.Integrations.Modules.ENAV.ERP.Invoice.RetryingPolicyAction: retry policy expired maximum tentatives"), LogCategories);
        ...
        private async Task WorkflowPayableInvoiceUDSBuildCompleteCallback(IEventCompleteUDSBuild evt)
        {
            try
            {
                ...
                if (!workflowResult.IsValid)
                {
                    ...
                    throw new Exception("VecompSoftware.BPM.Integrations.Modules.ENAV.ERP.Invoice.WorkflowPayableInvoiceUDSBuildCompleteCallback");
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task WorkflowProtocolBuildCompleteCallback(IEventCompleteProtocolBuild evt)
        {
            try
            {
                ...
                {
                    _logger.WriteError(new LogMessage("An error occured in notify workflow activity"), LogCategories);
                    throw new Exception("VecompSoftware.BPM.Integrations.Modules.ENAV.ERP.Invoice.WorkflowProtocolBuildCompleteCallback");
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task WorkflowPECMailBuildCompleteCallback(IEventCompletePECMailBuild evt)
        {
            try
            {
                ...
                {
                    _logger.WriteError(new LogMessage("An error occured in notify workflow activity"), LogCategories);
                    throw new Exception("VecompSoftware.BPM.Integrations.Modules.ENAV.ERP.Invoice.WorkflowPECMailBuildCompleteCallback");
                }
            }
            catch (Exception ex)
            {
