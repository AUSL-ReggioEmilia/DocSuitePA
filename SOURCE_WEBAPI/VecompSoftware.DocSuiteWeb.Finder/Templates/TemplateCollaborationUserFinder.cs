...
namespace VecompSoftware.DocSuiteWeb.Finder.Templates
{
    public static class TemplateCollaborationUserFinder
    {
        ...
        {
            return repository.Query(f => f.Account == account && f.Role.IdRoleTenant == idRole)
        ...
        {
            return repository.Query(f => f.Role.IdRoleTenant == idRole).SelectAsQueryable().FirstOrDefault();
        }
