...
namespace VecompSoftware.DocSuiteWeb.Service.Workflow
{
    ...
    public class WorkflowValidateService : WorkflowBaseService<WorkflowValidate>, IWorkflowValidateService, IDisposable
    {
        #region [ Properties ]
        ...
        #region [ Constructor ]
        public WorkflowValidateService(ILogger logger, IWorkflowInstanceService workflowInstanceService,
        ...
        protected override async Task<WorkflowResult> BeforeCreateAsync(WorkflowValidate content)
        {
            // TODO: implentare metodi async nel client del WF
            await Task.Delay(1);
            WorkflowResult validationResult = new WorkflowResult();
            WorkflowManagement wf = new WorkflowManagement(ClientConfig);
            validationResult.IsValid = wf.Validate(content.WorkflowXaml);
