    ...
    internal static partial class Utilities
    {
            ...
            )
        {
            ...
            try
            {
                ...
                {
                    reader.Read();
                    reader.ReadStartElement(XmlElementNames.ErrorRootElement);
                    extendedError.ErrorCode = reader.ReadElementString(XmlElementNames.ErrorCode);
                    extendedError.ErrorMessage = reader.ReadElementString(XmlElementNames.ErrorMessage);
                    ...
                    do
                    {
                        ...
                        {
                            if (string.Compare(reader.LocalName, XmlElementNames.ErrorException, StringComparison.Ordinal) == 0)
                            {
                                // Need to read exception details - we have message and stack trace
                                reader.ReadStartElement(XmlElementNames.ErrorException);
                                extendedError.AdditionalDetails.Add(XmlElementNames.ErrorExceptionMessage,
                                    reader.ReadElementString(XmlElementNames.ErrorExceptionMessage));
                                extendedError.AdditionalDetails.Add(XmlElementNames.ErrorExceptionStackTrace,
                                    reader.ReadElementString(XmlElementNames.ErrorExceptionStackTrace));
        ...
        internal static StorageExtendedErrorInformation GetExtendedErrorFromXmlMessage(string xmlErrorMessage)
        {
            ...
            XName xnErrorCode = XName.Get(XmlElementNames.TableErrorCodeElement,
                XmlElementNames.DataWebMetadataNamespace);
            XName xnMessage = XName.Get(XmlElementNames.TableErrorMessageElement,
                XmlElementNames.DataWebMetadataNamespace);
