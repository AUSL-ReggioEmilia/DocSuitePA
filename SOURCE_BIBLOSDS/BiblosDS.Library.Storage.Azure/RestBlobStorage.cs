...
namespace BiblosDS.Library.Storage.Azure
{
    internal class BlobStorageRest : BlobStorage
    {
            ...
            )
        {
            ...
            HttpWebRequest request = Utilities.CreateHttpRequest(listContainerssUri, HttpMethod.Get, Timeout);
        ...
        internal static NameValueCollection CreateRequestUriForListing(string prefix, string fromMarker, string delimiter, int maxResults)
        {
            ...
            queryParams.Add(QueryParams.QueryParamComp, CompConstants.List);

            if (!string.IsNullOrEmpty(prefix))
                queryParams.Add(QueryParams.QueryParamPrefix, prefix);

            if (!string.IsNullOrEmpty(fromMarker))
                queryParams.Add(QueryParams.QueryParamMarker, fromMarker);

            if (!string.IsNullOrEmpty(delimiter))
                queryParams.Add(QueryParams.QueryParamDelimiter, delimiter);

            queryParams.Add(QueryParams.QueryParamMaxResults,
        ...
        private static ListContainersResult ListContainersResultFromResponse(Stream responseBody)
        {
            ...
            foreach (XmlNode containerNode in containerNodes)
            {
                DateTime? lastModified = XPathQueryHelper.LoadSingleChildDateTimeValue(
                    containerNode, XmlElementNames.LastModified, false);
    ...
    internal class BlobContainerRest : BlobContainer
    {
        ...
        public override bool DoesContainerExist()
        {
            ...
            RetryPolicy(() =>
            {
                ...
                HttpWebRequest request = Utilities.CreateHttpRequest(uri, HttpMethod.Get, Timeout);
                credentials.SignRequest(request, uriComponents);
        ...
        public override ContainerAccessControl GetContainerAccessControl()
        {
            ...
            RetryPolicy(() =>
            {
                ResourceUriComponents uriComponents;
                NameValueCollection queryParams = new NameValueCollection();
                queryParams.Add(QueryParams.QueryParamComp, CompConstants.Acl);
                ...
                try
                {
                    using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
                    {
                        if (response.StatusCode == HttpStatusCode.OK)
                        {
                            string acl = response.Headers[HeaderNames.PublicAccess];
        ...
        public override void SetContainerAccessControl(ContainerAccessControl acl)
        {
            RetryPolicy(() =>
            {
                ...
                Uri uri = Utilities.CreateRequestUri(BaseUri, UsePathStyleUris, AccountName, ContainerName, null, Timeout,
                            queryParams, out uriComponents);
                HttpWebRequest request = Utilities.CreateHttpRequest(uri, HttpMethod.Put, Timeout);
                request.Headers.Add(HeaderNames.PublicAccess,
        ...
        public override bool DeleteContainer()
        {
            ...
            RetryPolicy(() =>
            {
                ...
                Uri uri = Utilities.CreateRequestUri(BaseUri, this.UsePathStyleUris, AccountName, ContainerName, null, Timeout, new NameValueCollection(),
                            out uriComponents);
                HttpWebRequest request = Utilities.CreateHttpRequest(uri, HttpMethod.Delete, Timeout);
                credentials.SignRequest(request, uriComponents);
                try
                {
                    using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
        ...
        private bool CreateContainerImpl(NameValueCollection metadata, ContainerAccessControl accessControl)
        {
            ...
            RetryPolicy(() =>
            {
                ...
                {
                    request.Headers.Add(HeaderNames.PublicAccess, "true");
                }
                credentials.SignRequest(request, uriComponents);
        ...
        {
            string marker = "";
            const int maxResults = ListingConstants.MaxBlobListResults;
                    ...
                    long contentLength, bool overwrite, string eTag)
        {
            ...
            if (blobProperties.ContentEncoding != null)
                request.Headers.Add(HeaderNames.ContentEncoding, blobProperties.ContentEncoding);
            if (blobProperties.ContentLanguage != null)
                request.Headers.Add(HeaderNames.ContentLanguage, blobProperties.ContentLanguage);
            ...
            if (!overwrite)
            {
                request.Headers.Set(HeaderNames.IfNoneMatch, "*");
            }
            return request;
        }
        ...
        private HttpWebRequest CreateHttpRequestForGetBlob(Uri uri, string httpMethod, string ifNoneMatchETag, string ifMatchETag)
        {
            ...
            {
                request.Headers.Add(HeaderNames.IfNoneMatch, ifNoneMatchETag);
            }
            if (ifMatchETag != null)
            {
                request.Headers.Add(HeaderNames.IfMatch, ifMatchETag);
            }
            return request;
        }
            ...
            try
            {
                using (Stream requestStream = request.GetRequestStream())
                {
                    ...
                    using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
                    {
                        ...
                        blobProperties.ETag = response.Headers[HeaderNames.ETag];
                        response.Close();
                    }
                    requestStream.Close();
                }
        ...
        private BlobProperties GetBlobImpl(string blobName, Stream stream, string oldETag, bool chunked, out bool modified)
        {
            ...
            if (chunked && blobProperties != null && blobProperties.ContentLength > 0)
            {
                ...
                {
                    long nBytes = Math.Min(blobProperties.ContentLength - location, BlobBlockConstants.BlockSize);
                    R(() =>
                    {
        ...
        private bool SetBlobMetadataImpl(BlobProperties blobProperties, string eTag)
        {
            ...
            {
                NameValueCollection queryParams = new NameValueCollection();
                queryParams.Add(QueryParams.QueryParamComp, CompConstants.Metadata);
        ...
        private static NameValueCollection MetadataFromHeaders(WebHeaderCollection headers)
        {
            ...
            {
                if (headerName.StartsWith(HeaderNames.PrefixForMetadata,
                            ...
                            )
        {
            ...
            blobProperties.ContentEncoding = response.Headers[HeaderNames.ContentEncoding];
            blobProperties.LastModifiedTime = response.LastModified.ToUniversalTime();
            blobProperties.ETag = response.Headers[HeaderNames.ETag];
            blobProperties.ContentLanguage = response.Headers[HeaderNames.ContentLanguage];
        ...
        private ContainerProperties ContainerPropertiesFromResponse(HttpWebResponse response)
        {
            ...
            prop.ETag = response.Headers[HeaderNames.ETag];
        ...
        bool DeleteBlobImpl(string name, string eTag, out bool modified)
        {
            ...
            RetryPolicy(() =>
            {
                ...
                Uri blobUri = Utilities.CreateRequestUri(BaseUri, this.UsePathStyleUris, AccountName, ContainerName, name, Timeout, new NameValueCollection(),
                                           out uriComponents);
                HttpWebRequest request = Utilities.CreateHttpRequest(blobUri, HttpMethod.Delete, Timeout);
        ...
        private ListBlobsResult ListBlobsImpl(string prefix, string fromMarker, string delimiter, int maxCount)
        {
            ...
            RetryPolicy(() =>
            {
                ...
                HttpWebRequest request = Utilities.CreateHttpRequest(listBlobsUri, HttpMethod.Get, Timeout);
                credentials.SignRequest(request, uriComponents);
        ...
        private static ListBlobsResult ListBlobsResultFromResponse(Stream responseBody)
        {
            ...
            foreach (XmlNode prefixNode in prefixNodes)
            {
                string blobPrefix = XPathQueryHelper.LoadSingleChildStringValue(
                    prefixNode, XmlElementNames.BlobPrefixName, false);
            ...
            foreach (XmlNode blobNode in blobNodes)
            {
                ...
                string eTag = XPathQueryHelper.LoadSingleChildStringValue(
                    blobNode, XmlElementNames.Etag, false);
                ...
                long? blobSize = XPathQueryHelper.LoadSingleChildLongValue(
                    blobNode, XmlElementNames.Size, false);
    ...
    internal static class XPathQueryHelper
    {
        ...
        internal static readonly string NextMarkerQuery = string.Join(ConstChars.Slash,
            new string[] 
            {
                "", "", 
        ...
        internal static readonly string ContainerQuery = string.Join(ConstChars.Slash,
            new string[] 
            {
                "", "", 
                XmlElementNames.EnumerationResults,
                XmlElementNames.Containers,
                XmlElementNames.Container
            });

        internal static readonly string BlobQuery = string.Join(ConstChars.Slash,
            new string[] 
            {
                "", "", 
                XmlElementNames.EnumerationResults,
                XmlElementNames.Blobs,
                XmlElementNames.Blob
            });

        internal static readonly string BlockQuery = string.Join(ConstChars.Slash,
            new string[] 
            {
                "", "", 
                XmlElementNames.BlockList,
                XmlElementNames.Block
            });

        internal static readonly string QueueListQuery = string.Join(ConstChars.Slash,
            new string[] 
            {
                "", "", 
                XmlElementNames.EnumerationResults,
                XmlElementNames.Queues,
                XmlElementNames.Queue
            });

        internal static readonly string MessagesListQuery = string.Join(ConstChars.Slash,
                    new string[] 
            {
                "", "", 
                XmlElementNames.QueueMessagesList,
                XmlElementNames.QueueMessage,
            });

        internal static readonly string CommonPrefixQuery = string.Join(ConstChars.Slash,
            new string[] 
            {
                "", "", 
                XmlElementNames.EnumerationResults,
                XmlElementNames.Blobs,
                XmlElementNames.BlobPrefix
            });
