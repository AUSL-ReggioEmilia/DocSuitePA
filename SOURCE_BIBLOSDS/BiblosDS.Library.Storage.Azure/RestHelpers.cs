...
namespace BiblosDS.Library.Storage.Azure
{
    ...
    internal static partial class Utilities
    {
        internal static HttpWebRequest CreateHttpRequest(Uri uri, string httpMethod, TimeSpan timeout)
        {
            ...
            request.Headers.Add(HeaderNames.StorageDateTime,
        ...
        internal static void AddMetadataHeaders(HttpWebRequest request, NameValueCollection metadata)
        {
            ...
            {
                request.Headers.Add(
                    HeaderNames.PrefixForMetadata + key,
                    metadata[key]
                    );
            }
        }
        ...
        internal static bool IsValidTableName(string name)
        {
            ...
            Regex reg = new Regex(RegularExpressionStrings.ValidTableNameRegex);
            if (reg.IsMatch(name))
            {
                return true;
            }
        ...
        internal static bool IsValidContainerOrQueueName(string name)
        {
            ...
            Regex reg = new Regex(RegularExpressionStrings.ValidContainerNameRegex);
            if (reg.IsMatch(name))
            {
                return true;
            }
