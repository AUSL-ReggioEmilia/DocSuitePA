...
namespace BiblosDS.Library.Storage.Azure
{
    ...
    public class TableStorage
    {
        ...
        public IEnumerable<string> ListTables()
        {
            ...
            // occurs
            query = query.Take(ListingConstants.MaxTableListResults);
            ...
            qor.Headers.TryGetValue(HeaderNames.PrefixForTableContinuation +
                                    HeaderNames.NextTableName,
                                    out nextKey);

            while (nextKey != null)
            {
                localQuery = orig;
                localQuery = localQuery.AddQueryOption(HeaderNames.NextTableName, nextKey);
                try
                {
    ...
    public class TableStorageDataServiceQuery<TElement>
    {
        ...
        public IEnumerable<TElement> ExecuteAll(bool withRetry)
        {
            ...
            qor.Headers.TryGetValue(HeaderNames.PrefixForTableContinuation +
                                    HeaderNames.NextPartitionKey,
                                    out nextPartitionKey);
            qor.Headers.TryGetValue(HeaderNames.PrefixForTableContinuation +
                                    HeaderNames.NextRowKey,
                                    out nextRowKey);
            ...
            while (nextPartitionKey != null || nextRowKey != null)
            {
                ...
                {
                    localQuery = localQuery.AddQueryOption(HeaderNames.NextPartitionKey, nextPartitionKey);
                }
                if (nextRowKey != null)
                {
                    localQuery = localQuery.AddQueryOption(HeaderNames.NextRowKey, nextRowKey);
                }
                if (withRetry)
                {
    ...
    internal class ContextRef
    {
        ...
        private void HandleSendingRequest(object sender, SendingRequestEventArgs e)
        {
            ...
            if (col != null)
            {
                ...
                {
                    if (string.Compare(header, HeaderNames.StorageDateTime, StringComparison.Ordinal) == 0)
                    {
                        return;
                    }
                }
