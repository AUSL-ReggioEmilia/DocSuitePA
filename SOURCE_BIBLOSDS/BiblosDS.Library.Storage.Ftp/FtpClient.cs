...
namespace BiblosDS.Library.Storage.Ftp
{
	...
	internal class FtpClient
	{
        ...
        public void Upload(string fileName, byte[] filelob, bool resume)
        {
            if (!this.loggedin) this.Login();
            ...
            if (resume && filelob.Length < offset)
            {
                // different file size
                Debug.WriteLine("Overwriting " + fileName, "FtpClient");
                offset = 0;
            }
            else if (resume && filelob.Length == offset)
            {
                // file done             
                Debug.WriteLine("Skipping completed " + fileName + " - turn resume off to not detect.", "FtpClient");
                return;
            ...
            for (int i = 0; i < filelob.Length; i+= buffer.Length)
            {
                Array.Copy(filelob, i, buffer, 0, buffer.Length);
                cSocket.Send(buffer, buffer.Length, 0);
            }
