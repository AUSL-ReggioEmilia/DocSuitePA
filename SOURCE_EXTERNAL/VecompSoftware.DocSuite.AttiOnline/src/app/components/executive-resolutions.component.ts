...
@Component({
    moduleId: module.id,
    selector: 'executive-resolutions',
    templateUrl: 'executive-resolutions.component.html',
    styleUrls: [
        'resolutions-results.component.css'
    ]
})
    
export class ExecutiveResolutionsComponent implements OnInit {
    ...
    ngOnInit() {
        if (!AppConfigService.settings.activeRoutes.executiveConsultation) {
            this.router.navigate(['error-page']);
            console.log('Nell\'URL è stata indicata una rotta inesistente.');
            this.isAllowedModality = false;
        }
        else {
            this.documentHandlerUrl = AppConfigService.settings.executiveDocumentHandlerUrl;
            this.isAllowedModality = true;
            this.proposerColumnVisible = AppConfigService.settings.executiveConsultationProposerColumnVisibility;
            this.serviceColumnVisbile = AppConfigService.settings.executiveConsultationServiceColumnVisibility;
        }
    }
    ...
    private loadResolutions(type: ResolutionType): void {
        ...
        this.resolutionService.getExecutiveResolutions(type, this.skip, this.pageSize, year, number, subject, adoptionDate, proposer).subscribe(response => this.successfulCallback(response), this.errorCallback);       
    }
    ...
    private countResolutions(type: ResolutionType): void {
        ...
        this.resolutionService.countExecutiveResolutions(type, this.skip, this.pageSize, year, number, subject, adoptionDate, proposer).subscribe(response => this.successfulCountCallback(response), this.errorCallback);
    }
