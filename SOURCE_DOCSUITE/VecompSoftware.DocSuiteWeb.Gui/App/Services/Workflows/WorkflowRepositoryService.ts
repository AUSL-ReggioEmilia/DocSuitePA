...
class WorkflowRepositoryService extends BaseService {
    ...
    getByEnvironment(environment: number, filters: string, anyEnv: boolean, documentRequired: boolean, documentUnitRequired: boolean, showOnlyNoInstanceWorkflows: boolean, showOnlyHasIsFascicleClosedRequired: boolean, callback?: (data: any) => any, error?: (exception: ExceptionDTO) => any): void {
        let url: string = this._configuration.ODATAUrl;
        url = url.concat(`/WorkflowRepositoryService.GetAuthorizedActiveWorkflowRepositories(environment=${environment},anyEnv=${anyEnv},documentRequired=${documentRequired},documentUnitRequired=${documentUnitRequired},showOnlyNoInstanceWorkflows=${showOnlyNoInstanceWorkflows},showOnlyHasIsFascicleClosedRequired=${showOnlyHasIsFascicleClosedRequired})?$orderby=Name`);
