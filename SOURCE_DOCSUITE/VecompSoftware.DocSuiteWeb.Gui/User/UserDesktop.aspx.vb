...
#Region " Fields "
    ...
    Public Const PageDematerialisationRequest As String = "UserDematerialisationRequest.aspx"
...
#Region " Methods "
    ...
    Private Sub InitializeMenu()
        ...
        If DocSuiteContext.Current.ProtocolEnv.UDSEnabled = True Then
            RadTreeMenu.Nodes.Add(CreateArchiveNode())
        End If
    ...
    Private Shared Function CreateCollaborationGeneralNode() As RadTreeNode
        ...
        If DocSuiteContext.Current.IsDocumentEnabled AndAlso DocSuiteContext.Current.DocumentEnv.IsEnvLogEnabled _
           AndAlso DocSuiteContext.Current.IsProtocolEnabled AndAlso DocSuiteContext.Current.ProtocolEnv.IsLogEnabled _
           AndAlso DocSuiteContext.Current.IsResolutionEnabled AndAlso DocSuiteContext.Current.ResolutionEnv.IsLogEnabled Then
            AddNode(node, New UserDesktopNodeAction("Generale", "GY", "N", PageDiarioComune, "", "../Comm/Images/FolderClose16.gif"), True)
            Return node
        End If

        If DocSuiteContext.Current.IsProtocolEnabled AndAlso DocSuiteContext.Current.ProtocolEnv.IsLogEnabled Then
    ...
    Public Function CreateCollaborationNode() As RadTreeNode
        ...
        If DocSuiteContext.Current.ProtocolEnv.DematerialisationEnabled Then
            ...
            If menuJson("Menu3").Nodes.Keys.Contains("FirstNode13") Then
                AddNode(node, New UserDesktopNodeAction(menu.Nodes("FirstNode13").Nodes("SecondNode2").Name, "FDF", "N", PageDematerialisationRequest, "", "../Comm/Images/FolderClose16.gif"), False)
            End If
        End If
        Return node
    End Function

    Private Sub CreateDiarySection()
        ...
        If DocSuiteContext.Current.ProtocolEnv.FascicleEnabled AndAlso menuJson("Menu7").Nodes.Keys.Contains("FirstNode3") Then
            ...
            AddNode(node, New UserDesktopNodeAction(menu.Nodes("FirstNode3").Nodes("SecondNode1").Name, "FDF", "N", PageFascicle, "Fasc", "../Comm/Images/FolderClose16.gif"), False)
        End If
    ...
    Public Function GetNodeActionFromRadTree(node As RadTreeNode) As UserDesktopNodeAction
        Try
            Dim model As New UserDesktopNodeAction()
            model.Title = node.Text
            model.ActionType = node.Value
            model.NodeType = node.Attributes.Item("NodeType")
            model.PageName = node.Attributes.Item("PageName")
            model.DocumentType = node.Attributes.Item("DocumentType")
            model.ImageUrl = node.ImageUrl
            Return model
        Catch
            Return Nothing
        End Try
    ...
    Private Shared Function CreateArchiveNode() As RadTreeNode
        Dim node As New RadTreeNode()
