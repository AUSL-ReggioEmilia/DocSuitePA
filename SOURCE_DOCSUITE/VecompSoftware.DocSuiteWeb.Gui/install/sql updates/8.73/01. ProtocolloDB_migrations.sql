...
GO
--#############################################################################
PRINT 'Versionamento database alla 8.73'
GO

EXEC dbo.VersioningDatabase N'8.73'
GO
...
GO
--#############################################################################
PRINT N'Creazione della colonna [MetadataRepositories] della tabella [dbo].[Dossiers]';
GO

ALTER TABLE [dbo].[Dossiers] ADD [IdMetadataRepository] [UniqueIdentifier] NULL;
GO

ALTER TABLE [dbo].[Dossiers]  WITH CHECK ADD  CONSTRAINT [FK_Dossiers_MetadataRepositories] FOREIGN KEY([IdMetadataRepository])
REFERENCES [dbo].[MetadataRepositories] ([IdMetadataRepository])
GO

ALTER TABLE [dbo].[Dossiers] CHECK CONSTRAINT [FK_Dossiers_MetadataRepositories]
GO
...
GO
--#############################################################################
PRINT N'Creazione della tabella [uds].[UDSUsers]';
GO

CREATE TABLE [uds].[UDSUsers](
	[IdUDSUser] [uniqueidentifier] NOT NULL,
	...
	CONSTRAINT [PK_UDSUser] PRIMARY KEY NONCLUSTERED
	(
		[IdUDSUser] ASC
...
CREATE CLUSTERED INDEX [IX_UDSUsers_RegistrationDate]
	ON [uds].[UDSUSers]([RegistrationDate] ASC)
GO
	
ALTER TABLE [uds].[UDSUsers] WITH CHECK ADD CONSTRAINT [FK_UDSUsers_UDSRepositories] FOREIGN KEY ([IdUDSRepository])
	REFERENCES [uds].[UDSRepositories] ([IdUDSRepository])
GO

ALTER TABLE [uds].[UDSUsers] CHECK CONSTRAINT [FK_UDSUsers_UDSRepositories]
GO
...
GO
--#############################################################################
PRINT N'Aggiunta colonna [AuthorizationType] alla tabella [uds].[UDSAuthorizations]';
GO

ALTER TABLE [uds].[UDSAuthorizations] ADD [AuthorizationType] [smallint] NULL
GO

UPDATE [uds].[UDSAuthorizations] SET [AuthorizationType] = 1
GO

ALTER TABLE [uds].[UDSAuthorizations] ALTER COLUMN [AuthorizationType] [smallint] NOT NULL
GO
...
--#############################################################################
PRINT N'Rename colonna [uds].[UDSAuthorizations].IdUDSAuthorization in [IdUDSRole]';
GO

EXEC sp_RENAME '[uds].[UDSAuthorizations].IdUDSAuthorization' , 'IdUDSRole', 'COLUMN'
GO
...
GO
--#############################################################################
PRINT N'Rename tabella [uds].[UDSAuthorizations] in [uds].[UDSRoles]';
GO

EXEC sp_rename '[uds].[UDSAuthorizations]', 'UDSRoles'
GO
...
GO
--#############################################################################
PRINT N'Creata tabella [dbo].[ProtocolDocumentSeriesItem]';
GO

CREATE TABLE [dbo].[ProtocolDocumentSeriesItems](
	[IdProtocolDocumentSeriesItem] [uniqueidentifier] NOT NULL,
	[Year] [smallint] not null,
	[Number] [int] not null,
	[UniqueIdProtocol] [uniqueidentifier] not NULL,
	[IdDocumentSeriesItem] [int] not NULL,
	[UniqueIdDocumentSeriesItem] [uniqueidentifier] not NULL,
	[RegistrationUser] [nvarchar](256) NOT NULL,
	[RegistrationDate] [datetimeoffset](7) NOT NULL,
	[LastChangedUser] [nvarchar](256) NULL,
	[LastChangedDate] [datetimeoffset](7) NULL,
	[Timestamp] [timestamp] not null
 CONSTRAINT [PK_ProtocolDocumentSeriesItems] PRIMARY KEY NONCLUSTERED 
(
	[IdProtocolDocumentSeriesItem] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
) ON [PRIMARY]
GO

CREATE CLUSTERED INDEX [IX_ProtocolDocumentSeriesItem_RegistrationDate] 
	ON [dbo].[ProtocolDocumentSeriesItems] ([RegistrationDate] ASC)
	GO

ALTER TABLE [dbo].[ProtocolDocumentSeriesItems]
ADD  CONSTRAINT [DF_ProtocolDocumentSeriesItems_RegistrationDate]  DEFAULT (sysdatetimeoffset()) FOR [RegistrationDate]
GO

ALTER TABLE [dbo].[ProtocolDocumentSeriesItems]  WITH CHECK ADD  CONSTRAINT [FK_ProtocolDocumentSeriesItems_Protocol] FOREIGN KEY([Year], [Number])
REFERENCES [dbo].[Protocol] ([Year], [Number])
GO
ALTER TABLE [dbo].[ProtocolDocumentSeriesItems] CHECK CONSTRAINT [FK_ProtocolDocumentSeriesItems_Protocol]
GO

ALTER TABLE [dbo].[ProtocolDocumentSeriesItems]  WITH CHECK ADD  CONSTRAINT [FK_ProtocolDocumentSeriesItems_DocumentSeriesItem] FOREIGN KEY([IdDocumentSeriesItem])
REFERENCES [dbo].[DocumentSeriesItem] ([Id])
GO
ALTER TABLE [dbo].[ProtocolDocumentSeriesItems] CHECK CONSTRAINT [FK_ProtocolDocumentSeriesItems_DocumentSeriesItem]
GO

ALTER TABLE [dbo].[ProtocolDocumentSeriesItems]  WITH CHECK ADD CONSTRAINT [FK_ProtocolDocumentSeriesItems_Protocol_UniqueId] FOREIGN KEY([UniqueIdProtocol])
REFERENCES [dbo].[Protocol] ([UniqueId])
GO
ALTER TABLE [dbo].[ProtocolDocumentSeriesItems] CHECK CONSTRAINT [FK_ProtocolDocumentSeriesItems_Protocol_UniqueId]
GO

ALTER TABLE [dbo].[ProtocolDocumentSeriesItems]  WITH CHECK ADD CONSTRAINT [FK_ProtocolDocumentSeriesItems_DocumentSeriesItem_UniqueId] FOREIGN KEY([UniqueIdDocumentSeriesItem])
REFERENCES [dbo].[DocumentSeriesItem] ([UniqueId])
GO
ALTER TABLE [dbo].[ProtocolDocumentSeriesItems] CHECK CONSTRAINT [FK_ProtocolDocumentSeriesItems_DocumentSeriesItem_UniqueId]
GO

CREATE UNIQUE INDEX IX_ProtocolDocumentSeriesItems_Year_Number_IdDocumentSeriesItem
ON [dbo].[ProtocolDocumentSeriesItems] ([Year], [Number], [IdDocumentSeriesItem])
GO
...
GO
--#############################################################################
PRINT N'Aggiunta colonna [JsonParameters] alla tabella [TemplateCollaborations]';
GO

ALTER TABLE [dbo].[TemplateCollaborations] ADD JsonParameters [nvarchar](max) null
GO
...
--#############################################################################
PRINT N'Rimozione colonna [DocumentStream] alla tabella [PosteOnLineRequest]';
GO

ALTER TABLE [dbo].[PosteOnLineRequest] DROP COLUMN DocumentStream
GO
...
GO
--#############################################################################
PRINT N'Rimozione colonna [DocumentPosteStream] alla tabella [PosteOnLineRequest]';
GO

ALTER TABLE [dbo].[PosteOnLineRequest] DROP COLUMN DocumentPosteStream
GO
...
UPDATE uds.UDSSchemaRepositories set SchemaXML = '<xs:schema id="SchemaUnitaDocumentariaSpecifica"
    ...
    <xs:complexContent>
      <xs:extension base="BaseType">
        <xs:sequence>
          ...
          <xs:element name="CSSClassName" type="xs:string" minOccurs="0" maxOccurs="1" />
        </xs:sequence>
 ...
 <!-- Definizione della tipologia di autorizzazione-->
  ...
  <xs:complexType name="ProtocolInstance">
    <xs:attribute name="IdProtocol" type="xs:string" use="required" />
    <xs:attribute name="UniqueId" type="xs:string" />
    <!--Descrivere il resto-->
  </xs:complexType>
  ...
  <xs:complexType name="LookupField">
    <xs:complexContent>
      <xs:extension base="FieldBaseType">
        ...
        <xs:attribute name="LookupArchiveName" type="xs:string"/>
        <xs:attribute name="LookupArchiveColumnName" type="xs:string"/>
...
GO
--#############################################################################
PRINT N'Aggiunta vincolo unicita idUDS, Account alla tabella [UDSUsers]';
GO

ALTER TABLE [uds].[UDSUsers] ADD CONSTRAINT [DF_UDSUser_UniqueAuthorizedUser] UNIQUE (IdUDS, Account);
GO
