...
GO
--#############################################################################
PRINT 'Versionamento database alla 8.69'
GO

EXEC dbo.VersioningDatabase N'8.69'
GO
...
GO
--#############################################################################
PRINT 'Aggiunta colonna InvoiceYear in dbo.AdvancedProtocol'
GO

ALTER TABLE [dbo].[AdvancedProtocol] ADD [InvoiceYear] AS (YEAR(InvoiceDate)) PERSISTED
GO
...
GO
--#############################################################################
PRINT 'Aggiornamento e inserimento record nella tabella FasciclePeriods'
GO

UPDATE [dbo].[FasciclePeriods] SET [PeriodName] = 'Annuale' WHERE [PeriodDays] = 365
GO

UPDATE [dbo].[FasciclePeriods] SET [PeriodName] = 'Semestrale' WHERE [PeriodDays] = 180
GO

INSERT INTO [dbo].[FasciclePeriods] ([IdFasciclePeriod], [PeriodName], [PeriodDays], [IsActive], [RegistrationUser], [RegistrationDate])
VALUES (NEWID(), 'Trimestrale', 90, 1, 'docsuite', GETUTCDATE())
GO

INSERT INTO [dbo].[FasciclePeriods] ([IdFasciclePeriod], [PeriodName], [PeriodDays], [IsActive], [RegistrationUser], [RegistrationDate])
VALUES (NEWID(), 'Mensile', 30, 1, 'docsuite', GETUTCDATE())
GO
...
GO
--#############################################################################
PRINT 'Aggiunta colonna DSWEnvironment alla tabella Fascicles'
GO

ALTER TABLE [dbo].[Fascicles] ADD DSWEnvironment INT NULL
GO
