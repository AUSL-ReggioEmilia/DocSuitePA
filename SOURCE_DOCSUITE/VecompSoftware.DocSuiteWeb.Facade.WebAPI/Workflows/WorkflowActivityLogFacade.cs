...
namespace VecompSoftware.DocSuiteWeb.Facade.WebAPI.Workflows
{
    public class WorkflowActivityLogFacade : FacadeWebAPIBase<WorkflowActivityLog, WorkflowActivityLogDao>
    {
        ...
        #region [ Constructor ]
        public WorkflowActivityLogFacade(ICollection<TenantModel> model)
            : base(model.Select(s => new WebAPITenantConfiguration<WorkflowActivityLog, WorkflowActivityLogDao>(s)).ToList())
        {
        }
        #endregion
